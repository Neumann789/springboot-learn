 ...
 public class ControlOperatorParser
 {
   ...
   private static Pattern redirectionNoPipelinePattern = Pattern.compile("(>)|(<)");
   ...
   public static List<ConsoleOperation> findAllControlOperators(String buffer)
   {
     ...
     while (matcher.find())
     {
       ...
       if (matcher.group(5) != null) {
         reOpList.add(new ConsoleOperation(ControlOperator.OVERWRITE_OUT, 
           removeJavOSizeEscapeCharacters(buffer.substring(0, matcher.start(5)))));
         ...
         reOpList.add(new ConsoleOperation(ControlOperator.OVERWRITE_IN, 
           removeJavOSizeEscapeCharacters(buffer.substring(0, matcher.start(6)))));
         ...
         reOpList.add(new ConsoleOperation(ControlOperator.PIPE, 
           removeJavOSizeEscapeCharacters(buffer.substring(0, matcher.start(8)))));
     ...
     if (reOpList.size() == 0)
       reOpList.add(new ConsoleOperation(ControlOperator.NONE, removeJavOSizeEscapeCharacters(buffer)));
