	...
	public void accept(SignatureVisitor paramSignatureVisitor) {
		...
		if (str.charAt(0) == '<') {
			...
			do {
				...
				for (j = a(str, j, paramSignatureVisitor.visitClassBound()); (m = str.charAt(j++)) == ':'; j = a(str, j,
						paramSignatureVisitor.visitInterfaceBound())) {
				}
			} while (m != 62);
		} else {
			j = 0;
		...
		if (str.charAt(j) == '(') {
			...
			for (j = a(str, j + 1, paramSignatureVisitor.visitReturnType()); j < i; j = a(str, j + 1,
					paramSignatureVisitor.visitExceptionType())) {
			}
		}
		for (j = a(str, j, paramSignatureVisitor.visitSuperclass()); j < i; j = a(str, j,
				paramSignatureVisitor.visitInterface())) {
		}
	}
