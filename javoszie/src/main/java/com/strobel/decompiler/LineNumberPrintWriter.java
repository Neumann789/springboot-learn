   ...
   extends PrintWriter
 {
   ...
   {
     super(w);
     String maxNumberString = String.format("%d", new Object[] { Integer.valueOf(maxLineNo) });
     int numberWidth = maxNumberString.length();
     this._format = ("/*%" + numberWidth + "d*/");
     String samplePrefix = String.format(this._format, new Object[] { Integer.valueOf(maxLineNo) });
   ...
   private void doPrefix(int lineNumber) {
     if ((this._needsPrefix) && (!this._suppressLineNumbers)) {
       if (lineNumber == -1) {
         ...
         String prefix = String.format(this._format, new Object[] { Integer.valueOf(lineNumber) });
         super.print(prefix);
       }
     }
