   ...
   extends ContextTrackingVisitor<Void>
 {
   ...
   public Void visitLambdaExpression(LambdaExpression node, Void data)
   {
     ...
     if ((callSite != null) && (callSite.getBootstrapArguments().size() >= 3) && ((callSite.getBootstrapArguments().get(2) instanceof IMethodSignature)))
     {
       ...
       IMethodSignature signature = (IMethodSignature)callSite.getBootstrapArguments().get(2);
       
       if ((signature.getParameters().size() == 1) && (((ParameterDefinition)signature.getParameters().get(0)).getParameterType().getSimpleType() == JvmType.Integer) && (signature.getReturnType().isArray()) && (!signature.getReturnType().getElementType().isGenericType()))
       {
         ...
         if (match.success()) {
           ...
           if (signature.getReturnType().getElementType().isEquivalentTo(type.toTypeReference())) {
             MethodGroupExpression replacement = new MethodGroupExpression(node.getOffset(), new TypeReferenceExpression(-34, type.clone().makeArrayType()), "new");
