...
public class XML
{
	...
	for (int i = 0; i < length; i++) {
		...
		throw new JSONException(new StringBuilder().append("'").append
					    (string).append
	...
	(XMLTokener x, JSONObject context, String name) throws JSONException {
	...
	return (tagName == null
		? new StringBuilder().append("\"").append(string).append
		      ("\"").toString()
		: string.length() == 0
		? new StringBuilder().append("<").append(tagName).append
		      ("/>").toString()
		: new StringBuilder().append("<").append(tagName).append
		      (">").append
		      (string).append
