/* ClassNameFilter - Decompiled by JODE
...
{
    public static boolean validateClassName(String className, boolean isRegex,
					    String[] filters) {
	if (filters == null || filters.length <= 1)
	    return validateClassName(className, isRegex,
	...
	if (filter.length() > 2 && filter.startsWith("*")
	    && filter.endsWith("*"))
	    return className.contains(filter.substring(1,
						       filter.length() - 1));
	if (filter.length() >= 2 && filter.endsWith("*"))
	    return className.startsWith(filter.substring(0,
							 filter.length() - 1));
	if (filter.length() >= 2 && filter.startsWith("*"))
