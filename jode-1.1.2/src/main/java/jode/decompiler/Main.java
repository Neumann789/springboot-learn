...
public class Main extends Options {
	...
	if (failedClasses.size() > 0) {
	    ...
	    Enumeration enum0 = failedClasses.elements();
	    while (enum0.hasMoreElements()) {
		GlobalOptions.err.println("\t" + enum0.nextElement());
	    }
	...
	if (destDir == null)
		...
		if ((params[i].endsWith(".jar") || params[i].endsWith(".zip"))
		    ...
		    ClassInfo.setClassPath(params[i]
					   + Decompiler.altPathSeparatorChar
					   + classPath);
		    Enumeration enum0 = new ZipFile(params[i]).entries();
		    while (enum0.hasMoreElements()) {
			String entry
			    = ((ZipEntry) enum0.nextElement()).getName();
