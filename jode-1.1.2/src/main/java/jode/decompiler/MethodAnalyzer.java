...
public class MethodAnalyzer implements Scope, ClassDeclarer {
	...
	if (innerAnalyzers != null) {
	    for (Enumeration enum0 = innerAnalyzers.elements();
	    		enum0.hasMoreElements(); ) {
		ClassAnalyzer classAna = (ClassAnalyzer) enum0.nextElement();
        ...
        for (Enumeration enum0 = allLocals.elements();
        		enum0.hasMoreElements(); ) {
            LocalInfo li = (LocalInfo)enum0.nextElement();
            if (!li.isShadow())
    ...
    public LocalInfo findLocal(String name) {
        Enumeration enum0 = allLocals.elements();
	...
	if (innerAnalyzers != null) {
	    Enumeration enum0 = innerAnalyzers.elements();
	    while (enum0.hasMoreElements()) {
		ClassAnalyzer classAna = (ClassAnalyzer) enum0.nextElement();
		if (classAna.getParent() == this
