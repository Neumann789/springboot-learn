...
// 				  +shadow.name+"("+shadow.type+")");
	...
	java.util.Enumeration enum0 = operators.elements();
	while (enum0.hasMoreElements()) {
	    LocalVarOperator lvo = 
		(LocalVarOperator) enum0.nextElement();
	    if (needTypeUpdate) {
	...
	enum0 = hints.elements();
	while (enum0.hasMoreElements()) {
	    Object hint = enum0.nextElement();
	...
	if (name == null) {
	    Enumeration enum0 = hints.elements();
	    while (enum0.hasMoreElements()) {
		Hint hint = (Hint) enum0.nextElement();
	...
	if (newType == Type.tError
        ...
        if (!li.type.equals(newType)) {
            li.type = newType;
            java.util.Enumeration enum0 = li.operators.elements();
            while (enum0.hasMoreElements()) {
                LocalVarOperator lvo = (LocalVarOperator) enum0.nextElement();
                if ((GlobalOptions.debuggingFlags & GlobalOptions.DEBUG_TYPES) != 0)
                    GlobalOptions.err.println("updating "+lvo);
                lvo.updateType();
            }
	...
	LocalInfo li = getLocalInfo();
	Enumeration enum0 = li.operators.elements();
	int writes = 0;
	while (enum0.hasMoreElements()) {
	    if (((LocalVarOperator) enum0.nextElement()).isWrite())
		writes++;
	}
