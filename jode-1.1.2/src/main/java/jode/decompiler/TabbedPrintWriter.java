...
public class TabbedPrintWriter {
	...
	public void dumpRegion(String line) {
	    ...
	    Enumeration enum0 = childBPs.elements();
	    int cur = startPos;
	    BreakPoint child = (BreakPoint) enum0.nextElement();
	    if (child.startPos >= 0) {
		...
		child = (BreakPoint) enum0.nextElement();
...
//  		       +space+","+lastSpace+";"+minPenalty);
	    ...
	    for (lastChild = (BreakPoint) enum0.nextElement();
	    		enum0.hasMoreElements(); lastChild = nextChild) {
		nextChild = (BreakPoint) enum0.nextElement();
