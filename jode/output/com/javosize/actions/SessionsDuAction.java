/* SessionsDuAction - Decompiled by JODE
...
public class SessionsDuAction extends Action
{
	...
	Object session = UserThreadSessionTracker.getSessionById(id);
	String size = new StringBuilder().append("").append
			  (Agent.getObjectDeepSize(session)).toString();
	report.addSection("Deep Session Size (bytes)", new String[] { size });
	return new StringBuilder().append(report.toString()).append("\n")
		   .toString();
    }
}
