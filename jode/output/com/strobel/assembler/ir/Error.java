...
final class Error
{
    ...
    public static RuntimeException stackMapperCalledWithUnexpandedFrame
	(FrameType frameType) {
	throw new IllegalStateException
		  (String.format
		   ("StackMappingVisitor.visitFrame() was called with an unexpanded frame (%s).",
		    new Object[] { frameType.name() }));
    }
    
    public static RuntimeException invalidBootstrapMethodEntry
	(MethodReference bootstrapMethod, int parameterCount,
	 int argumentCount) {
	if (argumentCount <= parameterCount + 3)
	    return (new IllegalStateException
		    (String.format
		     ("Invalid BootstrapMethods attribute entry: %d additional arguments required for method %s, but only %d specified.",
		      new Object[] { Integer.valueOf(parameterCount - 3),
				     bootstrapMethod.getFullName(),
				     Integer.valueOf(argumentCount) })));
	return (new IllegalStateException
		(String.format
		 ("Invalid BootstrapMethods attribute entry: %d too many arguments specifiedfor method %s.",
		  (new Object[]
		   { Integer.valueOf(argumentCount - (parameterCount + 3)),
		     bootstrapMethod.getFullName() }))));
    }
}
