/* InlineEscapingAssignmentsTransform - Decompiled by JODE
...
public class InlineEscapingAssignmentsTransform extends ContextTrackingVisitor
{
	...
	if (previous instanceof VariableDeclarationStatement && value != null
	    && !value.isNull()) {
	    VariableDeclarationStatement d
		= (VariableDeclarationStatement) previous;
	    AstNodeCollection variables = d.getVariables();
	    VariableInitializer initializer
		= (VariableInitializer) variables.firstOrNullObject();
	    Variable variable
		= (Variable) initializer.getUserData(Keys.VARIABLE);
	    if ((variable == null || variable.getOriginalVariable() == null
		 || !variable.getOriginalVariable().isFromMetadata())
		&& variables.hasSingleElement()
		&& value instanceof IdentifierExpression
		&& Pattern.matchString(initializer.getName(),
